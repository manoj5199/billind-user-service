...
export function generateKeyPair(bits = 2048) {
  try {
    console.log(
      `Generating a ${bits}-bit RSA key pair. This may take a moment...`
    );
...
export const decryptMessage = (encryptedMessage: string) => {
  try {
    ...
    const privateKey = forge.pki.privateKeyFromPem(privateKey_);
    ...
    const decryptedMessage = forge.util.decodeUtf8(decryptedData);
